...
public class ShoppingCartController {
	...
	public void removeProductFromCart(@PathVariable int userId, int productId) {
		shoppingCartService.removeProductFromCart(userId, productId);
	}
	
	@PutMapping("/user/{userId}/products/{productId}")
	public void updateQuantityOfProduct(@PathVariable int userId, @RequestBody Product product) {
		shoppingCartService.updateQuantityOfProduct(userId, product);
	}
	
	@GetMapping("/user/{cartId}/products")
